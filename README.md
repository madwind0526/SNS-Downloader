...
주의: PC Local/Phone via PC 모드는 Windows 로컬 서버를 같은 네트워크에서 바로 열 수 있습니다. 신뢰하는 개인 네트워크에서만 사용하세요.
...
- Render 쿠키 파일은 로그인 비밀번호 기반 key로 암호화되어 사용자별로 저장됩니다.
- Render에 `DATABASE_URL`을 설정하면 사용자/암호화 쿠키가 외부 DB에 저장되어 restart/deploy 후에도 유지됩니다.
- `DATABASE_URL`이 없으면 기존처럼 `server/data/` 파일 저장을 사용하며, Render Free에서는 restart/deploy 때 사라질 수 있습니다.
...
2. Supabase 또는 Neon에서 Postgres DB 생성 후 Render 환경변수 `DATABASE_URL` 설정
   - Supabase/Neon 연결은 기본 SSL을 사용합니다.
   - 로컬 Postgres처럼 SSL이 없는 DB는 `DATABASE_SSL=0`을 추가합니다.
3. 배포 후 `https://sns-downloader.onrender.com` 접속
4. 첫 사용자는 username `admin`으로 등록
5. 추가 사용자는 초대 코드로 등록
6. 로그인 후 상단 쿠키 버튼에서 본인 `cookies.txt` 업로드

`admin` 사용자는 설정 화면에서 사용자 목록, 비밀번호 초기화, 쿠키 삭제, 사용자 삭제를 관리할 수 있습니다.

Free Render의 기본 파일시스템은 deploy/restart 때 보존되지 않습니다. 장기 보존에는 `DATABASE_URL`을 설정한 Supabase/Neon Postgres 사용을 권장합니다. Turso는 SQLite 계열이라 별도 드라이버를 추가해야 합니다.
...
| `DATABASE_URL` | 선택 사항. Supabase/Neon Postgres 연결 문자열 |
...
| `GET` | `/api/settings/cookies/diagnostics` | 쿠키 저장/복호화/사용 해시 진단 |
| `GET` | `/api/admin/diagnostics/yt-dlp` | 관리자용 yt-dlp 최근 실패 진단 |
