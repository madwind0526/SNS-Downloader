...
- 외부 Render 화면에서 쿠키 설정을 쓰려면 Render 환경변수 `ACCESS_TOKEN`으로 비밀번호 보호를 켜야 합니다.
...
3. 비밀번호 입력
4. 상단 쿠키 버튼에서 `cookies.txt` 업로드

방법 2: 환경변수로 등록

Render 재시작/재배포 후에도 쿠키를 자동 복원하려면 `COOKIES_BASE64` 환경변수를 사용합니다.

```powershell
[Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes((Get-Content .\cookies.txt -Raw -Encoding UTF8)))
```

출력값을 Render Dashboard의 `COOKIES_BASE64`에 넣고 재배포합니다.
