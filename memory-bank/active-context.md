...
- SNS Downloader v1.21 Render info requests use cookies on the first attempt when cookies are registered.
- `/api/info` no longer starts cookie-less when user cookies already exist.
...
- Next deploy should verify `https://sns-downloader.onrender.com/api/version` returns `1.21`.
